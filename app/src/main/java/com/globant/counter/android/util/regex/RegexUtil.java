...
public class RegexUtil {
    ...
    private static final String DIVISION_ZERO_REGEX = ".*\\/0([^.]|$|\\.(0{4,}.*|0{1,4}([^0-9]|$))).*";
    private static final String MODULE_REGEX = "^-*[0-9,\\.]+\\%\\=$";
    private static final String SPLIT_REGEX = "[-+*/=%]";
    private static final String VALID_NUMER_REGEX = "^[0-9]+([,.][0-9]+)?$";
    ...
    public boolean isDivisionValid(String data){
        return data.matches(DIVISION_REGEX) && !data.matches(DIVISION_ZERO_REGEX);
    }

    public boolean isModuleValid(String data){
        return data.matches(MODULE_REGEX);
    }
